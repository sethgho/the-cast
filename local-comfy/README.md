...
Feeding two character plates to the model merges them, so **the layout is done in pixels**:
`ImageScaleBy` + `ImageCompositeMasked` place two transparent cutouts on a scene plate, and the
model is only asked to redraw that composite as one panel. `VAEEncode` of the composite is the
sampler's latent, so `POLISH` (denoise) controls how far it may stray.

- **0.2–0.4 is the working range.** At 0.55 the model invented a third face on a fence rail.
- **The action comes from the cutouts.** At a polish low enough to keep both characters on-model,
  it will not re-pose them — generate the pose in the pose app, save it with `TRANSPARENT PNG` on,
  drop it in `ComfyUI/input/`, then stage it here.
- A `PreviewImage` shows the composite before the model sees it — the fast way to fix an X/Y.

## Scene plates

`plates/cast-scene-*.png` — ten empty locations. Six are the canon plates from the repo; four were
generated by `make_plates.py`, which **edits an existing plate** rather than generating from text.
That is what keeps the ink, paper tone and palette identical across the library.
